...
def split_dataset(config, wav_paths, script_paths, valid_ratio=0.05):
    ...
    for i in range(config.workers):
        ...
        train_dataset_list.append(BaseDataset(
                                        ...
                                        SOS_token, EOS_token, use_mel=config.use_mel))
        train_begin = train_end 

    valid_dataset = BaseDataset(wav_paths[train_end_raw_id:], script_paths[train_end_raw_id:], SOS_token, EOS_token, use_mel=config.use_mel)
...
def main():
    ...
    # custom argment
    parser.add_argument("--use_mel", action='store_true', help='use melspectrogram for data (default: False)')
    parser.add_argument("--rnn_cell", type=str, default='gru', help='RNN type [lstm & rnn] (default: gru)')
    parser.add_argument("--loss", type=str, default='crossentropy', help='loss type [crossentropy & focal]')
    ...
    enc = EncoderRNN(feature_size, args.hidden_size,
                     ...
                     n_layers=args.layer_size, bidirectional=args.bidirectional, rnn_cell=args.rnn_cell, variable_lengths=False, use_mel=args.use_mel)
    ...
    # optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=0.025)
    optimizer = RAdam(model.parameters(), lr=args.lr, weight_decay=0.025)
    # optimizer = optim.Adam(model.module.parameters(), lr=args.lr, weight_decay=0.025)
    # eta_min = 1e-6
    # T_max = 10
    # T_mult = 1
    # restart_decay = 0.97
    ...
    if args.loss == 'crossentropy':
        ...
        criterion = FocalLoss(reduction='sum', gamma=2, ignore_index=PAD_token).to(device)
    ...
    for epoch in range(begin_epoch, args.max_epochs):
        ...
        nsml.save(args.save_name + '_' + str(epoch))
