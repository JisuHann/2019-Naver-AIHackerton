...
class EncoderRNN(BaseRNN):
    ...
    def __init__(self, feature_size, hidden_size,
                 ...
                 n_layers=1, bidirectional=False, rnn_cell='gru', variable_lengths=False, use_mel=False):
    ...
    def forward(self, input_var, input_lengths=None):
        ...
        input_var = input_var.unsqueeze(1) # (B, 1, time, 128)
        x = self.conv(input_var) # (B, c, time, depth)

        # BxCxTxD => BxTxCxD
        x = x.transpose(1, 2) # (B, time, C, depth)
        x = x.contiguous()
        sizes = x.size()
